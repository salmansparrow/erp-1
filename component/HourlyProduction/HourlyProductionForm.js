  ...
  const handleSaveLines = async () => {
    ...
    const payload = {
      ...
      lines: linesToSave.map((line) => ({
        ...line,
        hourlyData: [], // Ensure hourlyData is empty during line save
      })),
    };
  ...
  const handleSaveHourlyData = async (hourIndex) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setLines((prevLines) =>
          prevLines.map((line) => {
            line.hourlyData[hourIndex].isSaved = true; // Mark saved
            return line;
          })
        );
      } else {
  ...
  return (
    <Box sx={{ padding: 5, paddingTop: 10 }}>
      ...
      <Box sx={{ marginTop: 4, textAlign: "right" }}>
        <Button variant="contained" color="primary" onClick={handleSaveLines}>
          Save All Lines
        </Button>
      </Box>
